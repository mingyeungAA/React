...
function PostDetail({posts, setPosts}){
    ...
    const [title, setTitle] = useState(post ? post.ttle : "");
        ...
        <div>
            {isEditing ? (
                <>
                    ...
                    <button onClick={() => setIsEditing(false)} style={{padding: "8px 12px", marginLeft: "10px"}}>취소</button>
                </>
            ) : (
                <>
                    <h1>{post.title}</h1>
                    ...
                    <button onClick={handelDelete} style={{padding: "8px 12px", marginLeft: "10px", color: "red"}}>삭제</button>
                    <button onClick={() => navigate("/")} style={{padding: "8px 12px", marginLeft: "10px"}}>목록으로</button>
                </>
            )}
        </div>
    );
